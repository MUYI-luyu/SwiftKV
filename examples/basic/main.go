...
func ConcurrencyExample() {
	...
	val1, ver1, _ := c1.Get("counter")
	fmt.Printf("  1. Get('counter') => '%s' (v%d)\n", val1, ver1)

	// 客户端 2
	fmt.Println("客户端 2:")
	c2 := rsm.MakeClerk(servers)
	val2, ver2, _ := c2.Get("counter")
	fmt.Printf("  1. Get('counter') => '%s' (v%d)\n", val2, ver2)

	// 客户端 1 尝试更新
	fmt.Println("客户端 1:")
	err := c1.Put("counter", "1", ver1)
	fmt.Printf("  2. Put('counter', '1', v%d) => %v\n", ver1, err)

	// 客户端 2 尝试用旧版本更新（应该失败）
	fmt.Println("客户端 2:")
	err = c2.Put("counter", "2", ver2)
	fmt.Printf("  2. Put('counter', '2', v%d) => %v (期望 ErrVersion)\n", ver2, err)

	// 客户端 2 重新读取并更新
	fmt.Println("客户端 2:")
	val2New, ver2New, _ := c2.Get("counter")
	fmt.Printf("  3. Get('counter') => '%s' (v%d)\n", val2New, ver2New)
	err = c2.Put("counter", "2", ver2New)
	fmt.Printf("  4. Put('counter', '2', v%d) => %v\n", ver2New, err)
...
func ErrorHandlingExample() {
	...
	_, version, _ := clerk.Get("test")
	clerk.Put("test", "changed", version) // 更新使版本号变化

	err = clerk.Put("test", "wrong", version) // 使用过时版本
	fmt.Printf("  Put('test', 'wrong', v%d) => %v\n", version, err)
	...
	_, version, _ = clerk.Get("test")
	err = clerk.Put("test", "attempt1", version)
	if err == kvapi.OK {
		_, version, _ = clerk.Get("test")
		// 使用更新前的版本重试
		err = clerk.Put("test", "attempt2", version)
		if err == kvapi.ErrVersion {
			// 再次尝试（视为重试）
			err = clerk.Put("test", "attempt3", version)
			fmt.Printf("  Put('test', 'attempt3', v%d) => %v\n", version, err)
...
func PersistenceExample() {
	...
	clerk1.Put("persistent_key", "important_data", 0)
	fmt.Println("  Put('persistent_key', 'important_data', v0) 成功")
	fmt.Println("")

	// 第二个客户端读取数据
	fmt.Println("[步骤 2] 第二个客户端读取数据")
	clerk2 := rsm.MakeClerk(servers)
	value, _, _ := clerk2.Get("persistent_key")
	fmt.Printf("  Get('persistent_key') => '%s'\n", value)
	fmt.Println("")
